...
GRAY = "#8a8f96"
LAUNCH = "#f6d5b4"  # 발사선·발사각 각도판 (연해야 바닥 센서가 흰 바닥으로 읽는다)
...
# ── 0도 기준선 강조 ──────────────────────────────────────────────
# 예전에는 여기가 유일한 '도착 지점'이었다. 이제 도착 눈금은 발사각을 따라
# 움직이므로, 정면(90°) 발사의 도착점이자 눈금의 0점이라는 표시로만 남긴다.
...
block('발사 규칙', BLUE, ['① 로켓은 발사대에, 머리는 각도판에',
                      '② 내행성(본부)이 발사선을 지나는',
                      '      순간 로켓 출발 + 스톱워치 시작',
                      '③ 조난자와 만나면(도킹) 시간 기록',
                      '④ 발사각을 바꿔 가며 가장 빨리',
...
block('시작 전 확인', ORANGE, ['· 로봇 속도는 선생님이 정합니다',
                          '· 정면(90°) 시험 발사로 방향을 확인한',
                          '   뒤 기록을 시작하세요'])
