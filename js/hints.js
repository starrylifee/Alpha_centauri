...
const Hints = (function () {
    ...
    const hintData = {
        ...
        3: {
            // 촬영 시간(24시간)은 학생이 직접 계산해야 하는 값이라 여기서 숫자로 알려주지 않는다
            level1: '별이 움직인 각도를 알면 사진을 몇 시간 찍었는지 거꾸로 구할 수 있습니다. 한 바퀴가 360도이고 이 행성의 하루가 288시간이라는 점을 이용하세요. 그렇게 나온 시간 내내 하늘이 어두웠다는 뜻이고, 대원이 어느 구역에 있는지가 여기서 나옵니다.',
            level2: '별들은 하늘의 한 점을 중심으로 원을 그립니다. 시뮬레이션 룸에서 그 중심점을 찾아, 지평선에서 몇 도 높이에 있는지 재세요. 그 값이 곧 그곳의 위도입니다.'
        },
        4: {
            level1: '시도마다 두 로봇을 출발점(둘 다 0° 자리)에 다시 놓고 동시에 출발시켰나요? 그래야 발사각만 바뀐 비교가 됩니다. 그리고 로켓이 날아가는 동안에도 조난자는 계속 돕니다. 지금 조난자가 있는 자리를 겨냥하면 도착했을 때 이미 지나가 버립니다. 어디를 겨냥해야 할까요?',
            level2: '정면(90°)으로 쏘면 로켓은 기준선 0°에 도착합니다. 조난자가 도는 방향 쪽으로 각도판을 조금 돌려 쏘면, 로켓이 도착할 무렵 조난자가 그 자리에 와 있어 기다리는 시간이 줄어듭니다. 시도마다 각도를 10~20°씩 바꿔 가며 도킹 시간이 어떻게 달라지는지 비교해 보세요.'
        }
    };
